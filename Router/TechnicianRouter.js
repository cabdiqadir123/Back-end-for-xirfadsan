...
TechnicianRouter.get('/all', (req, res) => {
    mysqlconnection.query('select * from technicians',
        (error, rows, fields) => {
            if (!error) {
                res.json(rows);
            } else {
...
TechnicianRouter.post('/add', (req, res) => {
    const {
        user_id,
        specialty,
        status,
    ...
    const sql = `
    INSERT INTO region (user_id,specialty ,status, created_at)
    VALUES (?,?, ?,?)
  `;
...
TechnicianRouter.put('/update/:id', (req, res) => {
...
TechnicianRouter.post('/delete/:id', (req, res) => {
    const id = req.params.id;
    console.log(req.body);
    mysqlconnection.query('delete from technicians where id=?'
