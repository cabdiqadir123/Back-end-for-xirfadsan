...
SupplierRouter.get('/all', (req, res) => {
    mysqlconnection.query('select supplier_id,user_id,suppliers.service_id,services.name as service_name,users.name,email,password,phone,address,dof,sex,role,status from users inner join suppliers on suppliers.user_id=users.id INNER JOIN services on services.service_id=suppliers.service_id',
        (error, rows, fields) => {
            if (!error) {
                res.json(rows);
            } else {
